    ...
    public class Basket : MonoBehaviour
    {
        ...
        private void OnDollEnterBasket(Collider dollCollider)
        {
            ...
            if (GameQuestManager.Instance.IsMissionTarget(dollName))
            {
                ...
                UIHud.CreateOrGet().GetDollTrailHud.PlayGetDollTrail(startPos, iconPath,
                    () => GameQuestManager.Instance.AddMission());
            }
            else
            {
                // 미션 무관: 랜덤박스 자리로 날아가 도착하면 랜덤박스 카운트 증가
                UIHud.CreateOrGet().GetDollTrailHud.PlayGetRandomBoxTrail(startPos, iconPath,
                    () => GameQuestManager.Instance.AddRandomBox());
            }
